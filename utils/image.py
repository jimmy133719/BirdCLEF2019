    ...
    while(count > 0 and len(augmentations) > 0):
        ...
        if RANDOM.choice([True, False], p=[probability, 1 - probability]):
            ...
            for key in augmentations.keys():
                keylist.append(key)
            #print(type(keylist))
            aug = np.random.choice(keylist,1).tolist()[0] #origin
#            aug = RANDOM.choice(augmentations.keys(),1)
            #print(aug)
...
def crop(img, top=0.1, left=0.1, bottom=0.1, right=0.1):
    ...
    print('t_crop = '+str(t_crop)+';l_crop = '+str(l_crop)+';b_crop = '+str(b_crop)+';r_crop'+str(r_crop))
...
if __name__ == '__main__':
    ...
    #cv2.imwrite('img.png',img*255.0)
#    showImage(img)                    
    img = augment(img, cfg.IM_AUGMENTATION, cfg.AUGMENTATION_COUNT, cfg.AUGMENTATION_PROBABILITY)
    cv2.imwrite('img_aug.png',img*255.0)
    top = 0.1   
    #cv2.imwrite('D:/BirdCLEF/train/spec/blhtan1/test.png',255*img)
    #showImage(img)
